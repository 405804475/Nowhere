...
- (instancetype)initWithPrecent:(NSString *)precent{
    [self setPrecent:precent];
    self.type = 2;
//    self.avgScoreLbl.text = [NSString stringWithFormat:@"%@",precent];
    return [[NSBundle mainBundle]loadNibNamed:@"SXWaveCell" owner:nil options:nil][0];
}
...
- (void)setPrecent:(NSString *)precent textColor:(UIColor *)tcolor type:(int)type alpha:(CGFloat)alpha
{
    [self setAlpha:alpha];
    [self setType:type];
...
#pragma mark - 修改了precent的类型
        CGFloat avgScore = 100;
